...
Функция ПолучитьПустуюТаблицуНаборЗаписей() Экспорт
	...
	НаборЗаписей 			= РегистрыСведений.ЖБИ_ЖурналСостоянийДокументов.СоздатьНаборЗаписей();	
...
Функция СоздатьОбновитьЗаписиРегистраСведений(ТаблицаНабораЗаписей, СписокСвойствОбновления = Неопределено, СтруктураПоиска = Неопределено, СообщениеОбОшибке) Экспорт
	...
	Для Каждого СтрокаНабораЗаписей Из ТаблицаНабораЗаписей Цикл	
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			КонецПопытки;;
		Иначе 
			МенеджерЗаписи = РегистрыСведений.ЖБИ_ЖурналСостоянийДокументов.СоздатьМенеджерЗаписи();
...
Функция ЗаполнитьЖурналСостоянийОбъектов(ОбъектСсылка,Состояние) Экспорт
	...
	НоваяСтр.Объект 		= ОбъектСсылка;
	НоваяСтр.Состояние      = Строка(Состояние);
	НоваяСтр.Автор 			= Пользователи.ТекущийПользователь();
	
	Результат 				= СоздатьОбновитьЗаписиРегистраСведений(ТаблицаНабораЗаписей,,, СообщениеОбОшибке);
